...
V10.38 is a visual-only treemap release. Upload `index.html`, `script.js`, `style.css`, `fresh-radar.css`, `layout-v10.34.css`, `service-worker.js`, `avatar-baby.jpg`, and the full `logos/` folder. The cost map now uses a squarified layout, dynamic height, minimum readable area for tiny holdings, adaptive logo/text levels, a dedicated mobile grid, and a monogram fallback for newly added symbols. It was verified with 1, 4, 8, 12, 18, 24, 32, and 48-stock scenarios plus rebalanced buy/sell cases. The price proxy, refresh rules, Worker, and live ledger are unchanged. Do not run `npx wrangler deploy` for this release, and do not upload `data.json`.
