...
V10.47 is a stability release. It adds per-symbol quote caching, a fixed configurable Twelve priority list, TWE/FIN backoff diagnostics, a frontend/PWA/Worker deployment fingerprint, and automated ledger/routing/treemap tests. Upload the changed code and test files, but do not upload or replace `data.json`, `backups/`, or quote cache files. This release changes `cloudflare-worker.js` and `wrangler.toml`, so deploy the Worker once after the GitHub code update.
