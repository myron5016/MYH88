...
function renderTreemap(){
  ...
  const items=treemapItems(),denom=Math.max(items.reduce((sum,item)=>sum+item.value,0),1);
  ...
  tiles.forEach((t,index)=>{
    ...
    d.innerHTML=`<div class="tile-copy">${showLogo?companyLogoMarkup(t.label):""}<div class="tile-text"><span class="tile-symbol">${escapeHtml(t.label)}${position?.shares<0?" · 空头":""}</span><span class="tile-meta">${escapeHtml(meta)}</span></div></div>`;
    box.appendChild(d)
  });
  hydrateSecurityLogos(box)
}
function sectorItems(){const map={};state.positions.forEach(p=>{const key=inferSector(p.symbol,p.name,p.sector);if(!map[key])map[key]={label:key,total:0,pnl:0,color:sectorBaseColor(key)};map[key].total+=Math.abs(num(p.costBasisUSD));map[key].pnl+=floatingPnlUSD(p)});const cash=cashBalance();if(cash>0)map["现金"]={label:"现金",total:cash,pnl:0,color:sectorBaseColor("现金")};return Object.values(map).sort((a,b)=>b.total-a.total)}
...
function transactionLabel(t){
  ...
  if(t?.type==="short")return"做空";
  if(t?.type==="cover")return"买入平空";
...
function switchLedgerTab(page,options={}){
  ...
  activeLedgerTab=tab;
  document.querySelectorAll("[data-ledger-page]").forEach(control=>{
    const active=control.dataset.ledgerPage===tab;
...
function initLedgerCarousel(){
  ...
  const end=event=>{
    if(!gestureStart||isLedgerGestureBlocked(event.target)){gestureStart=null;return}
    const finish=point(event),deltaX=finish.clientX-gestureStart.x,deltaY=finish.clientY-gestureStart.y;
    gestureStart=null;
