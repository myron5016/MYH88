...
function ledgerSummaryTradeAmountUSD(transaction){const shares=num(transaction.shares),price=num(transaction.price),fee=num(transaction.fee),rate=num(transaction.fxRate||1),gross=shares*price;return round((gross+(["buy","cover"].includes(transaction.type)?fee:-fee))*rate)}
...
function ledgerSummaryForMonth(year=ledgerSummaryMonth.year,month=ledgerSummaryMonth.month){
  const prefix=ledgerSummaryPrefix(year,month),validTransactions=(state.transactions||[]).filter(t=>!t?.voided&&String(t?.date||"").startsWith(prefix)&&["buy","sell","short","cover"].includes(t?.type));
  ...
  const buyUSD=validTransactions.filter(t=>["buy","cover"].includes(t.type)).reduce((sum,t)=>sum+ledgerSummaryTradeAmountUSD(t),0);
  const sellUSD=validTransactions.filter(t=>["sell","short"].includes(t.type)).reduce((sum,t)=>sum+ledgerSummaryTradeAmountUSD(t),0);
  ...
  const realizedPnlUSD=validTransactions.filter(t=>["sell","cover"].includes(t.type)).reduce((sum,t)=>sum+num(t.realizedPnlUSD),0);
  const recentTrades=validTransactions.slice().sort((a,b)=>String(b.date||"").localeCompare(String(a.date||""))||String(b.id||"").localeCompare(String(a.id||""))).slice(0,5).map(t=>({id:t.id||`${t.date}-${t.symbol}`,date:t.date,type:t.type,symbol:t.symbol||"—",name:t.name||t.symbol||"—",shares:num(t.shares),amountUSD:ledgerSummaryTradeAmountUSD(t),realizedPnlUSD:["sell","cover"].includes(t.type)?num(t.realizedPnlUSD):null,note:t.note||""}));
...
function renderLedgerSummary(){
  ...
  const metrics=[
    ["买入 / 平空金额",summary.buyUSD,"out"],
    ["卖出 / 做空金额",summary.sellUSD,"in"],
    ["追加本金",summary.depositUSD,"in"],
    ["已实现盈亏",summary.realizedPnlUSD,summary.realizedPnlUSD>=0?"in":"out"],
  ];
  grid.innerHTML=metrics.map(([label,value,direction])=>`<article class="ledger-summary-metric ${direction}"><span>${label}</span><strong>${money(value)}</strong><small>${label==="已实现盈亏"?"来自本月卖出及平空记录":"按美元统一统计"}</small></article>`).join("");
  const netClass=summary.netCashUSD>0?"in":summary.netCashUSD<0?"out":"flat";
  net.innerHTML=`<div class="ledger-summary-net-copy"><span>本月净现金变化</span><strong class="${netClass}">${ledgerSummarySignedMoney(summary.netCashUSD)}</strong><small>卖出 / 做空 + 追加本金 − 买入 / 平空 − 提取本金</small></div><div class="ledger-summary-net-breakdown"><span>提取本金 <b>${money(summary.withdrawUSD)}</b></span><span>共 ${summary.recentTrades.length} 笔交易</span></div>`;
  recent.innerHTML=summary.recentTrades.length?summary.recentTrades.map(trade=>`<div class="ledger-summary-row"><div><b class="${["buy","cover"].includes(trade.type)?"out":"in"}">${({buy:"买入",sell:"卖出",short:"做空",cover:"买入平空"})[trade.type]}</b><strong>${escapeHtml(trade.symbol)}</strong><small>${escapeHtml(trade.name)} · ${escapeHtml(trade.date||"")}</small></div><span>${["buy","cover"].includes(trade.type)?"-":"+"}${money(trade.amountUSD)}</span></div>`).join(""):"<p class=\"ledger-summary-empty\">本月还没有买入或卖出记录。</p>";
  syncLedgerHeight();
}
