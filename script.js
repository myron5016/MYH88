...
async function refreshPrices(useCache=true){
  ...
  try{
    ...
    state.settings.lastPriceRefresh=Date.now();state.settings.lastPriceRefreshText=new Date().toLocaleString("zh-CN");savePriceCache();captureSnapshot(false);markDirty("实时行情与今日快照已更新");renderAll();status.textContent="已刷新："+state.settings.lastPriceRefreshText+"。保存到 GitHub 后家人可见";
...
function transactionLabel(t){if(t.voided)return"已撤销";return t.type==="buy"?"买入":t.type==="sell"?"卖出":"V8 期初"}
